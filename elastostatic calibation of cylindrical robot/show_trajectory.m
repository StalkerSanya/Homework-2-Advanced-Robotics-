...
for i = 1:iter
    ...
    % random joint values
    q1 = robot.limits(1,2)*rand(1);
    q2 = robot.limits(2,2)*(2*rand(1)-1);
    q3 = robot.limits(3,2)*rand(1);
    q0 = [q1,q2,q3];
    % displacement    
...
for i = 1:NP
    conf = IK([XX(i);YY(i);ZLVL;0;0;0], robot);
    %conf(4:6) = [0;0;0];
    JSh(:,i) = conf;
    
    conf = IK([XX(i);YLVL;YY(i);0;0;0], robot);
    %conf(4:6) = [0;0;0];
    JSv(:,i) = conf;    
end
